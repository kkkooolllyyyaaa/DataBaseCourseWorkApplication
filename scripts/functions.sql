CREATE OR REPLACE FUNCTION accessory_with_serial_number_trigger()
    RETURNS TRIGGER
    LANGUAGE plpgsql AS
$$
DECLARE
...
BEGIN
    insert into serial_number (id) values (NEW.serial_number) ON CONFLICT DO NOTHING;
...
CREATE OR REPLACE FUNCTION car_part_with_serial_number_trigger()
    RETURNS TRIGGER
    LANGUAGE plpgsql AS
$$
DECLARE
...
CREATE OR REPLACE FUNCTION consumable_with_serial_number_trigger()
    RETURNS TRIGGER
    LANGUAGE plpgsql AS
$$
DECLARE
